    ...
    A_Boolean            *handledPB)                /* << */
{
    ...
    if (S_Grabba_cmd == command){
        ...
        if (active_itemH){
            ...
            suites.FootageSuite5()->AEGP_GetFootagePath(footageH, 0, 0, memHandle);
            
//            std::cout << "Found this footage:";
//            std::cout << footageH;
            
//            if (!err && roH){
//                AEGP_FrameReceiptH    receiptH     = NULL; // This makes it render and gives you a reference to the rendered data??
//                AEGP_WorldH            frameH        = NULL; // These are the actual pixels
//                A_Time                timeT        = {0,1}; // Piss
//                AEGP_WorldType        type        = AEGP_WorldType_NONE;
//
//                PF_Pixel8 **base_addrP = NULL;
//
...
//
//                    AEGP_WorldSuite3().AEGP_GetBaseAddr8(frameH, base_addrP);
//
//
////                    AEGP_GetBaseAddr8(
////                      AEGP_WorldH  worldH,
    ...
    AEGP_GlobalRefcon        *global_refconV)    /* << */
{
    ...
    if (err){ // not !err, err!
        ERR2(suites.UtilitySuite3()->AEGP_ReportInfo(S_my_id, "Shade : Could not register command hook."));
    }
    return err;
}
